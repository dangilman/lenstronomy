...
class Solver4Point(object):
    ...
    def __init__(self, lensModel):
        ...
        if not lensModel.lens_model_list[0] in ['SERSIC', 'SIS']:
    ...
    def solve(self, x_pos, y_pos, init, kwargs_list, a, xtol=1.49012e-10):
        options = {'xtol': xtol}
        x = scipy.optimize.minimize(self._F, init, args=(x_pos, y_pos, kwargs_list, a),
                                    method='Nelder-Mead', options=options)#, factor=0.1)
        return x
